...
from src.data.data_preprocess import preprocess_data            # Basic data cleaning
...
from src.models.model_tune import tune_model                     # Hyperparameter tuning (optional, can be skipped for faster runs)
...
def main(args):
    ...
    with mlflow.start_run():
        ...
        y = y.map({'No': 0, 'Yes': 1})  # Ensure target is binary 0/1 for XGBoost
        ...
        model = XGBClassifier(
            ...
            n_estimators=351,        # Number of trees (OPTIMIZED)
            learning_rate=0.0108,     # Step size shrinkage (OPTIMIZED)  
            max_depth=3,            # Maximum tree depth (OPTIMIZED)
            
            # Regularization parameters
            subsample=0.95,         # Sample ratio of training instances
            colsample_bytree=0.538,  # Sample ratio of features for each tree
